...
to c-sense ;sense of cars and adjust velocity
  ...
  let dc ceiling ( distance min-one-of cars [ distance myself ] - 2 * car-size ) / scale
  ifelse nc >= 1 and dc > 0 [ set c-veloc c-veloc * (random-float ( dc /  nc  ) ) ] [ set c-veloc c-veloc-max ]
end

to c-decide ;sense of pedestrians
  let np count peds in-cone (car-look-ahead-peds / scale) alfaVP ;10
  let dp ceiling ( distance min-one-of peds [ distance myself ] - car-size * 0.5 ) / scale
  ;adjust velocity
